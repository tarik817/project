 ...
 if(isset($_GET['display'])){
	...
	?>
 	...
 	<p>
 			...
 			<div><p><?php t("Дата реєстрації"); ?>:<br> <?php echo date('F j, Y, g:i a',$entry['users_data']) ?></p></div>
			<div><p><?php t("Last Log in"); ?>:<br> <?php echo date('F j, Y, g:i a',$entry['u_log']) ?></p></div>
			<p><a class="bott" href="?id=<?php echo $entry['users_name'] ?>"><?php t("View user information"); ?></a></p>
			<?php
			if(isset($_SESSION['admin'])){
			?>
			<div class ="u_del">
				<p class = "bott" ><a  href="user.php?ed=<?php echo $entry['users_name'] ?>"><?php t("Etite"); ?></a></p>
				<p class = "bott"><a href="users/u_del.php?del=<?php echo $entry['users_id'] ?>"><?php t("Delete"); ?></a></p>
			</div>
			<?php
			}
			?>
...
<form method ="post" action ="users/u_controler.php"> 
	...
	if(isset($_SESSION['admin'])){
	?>
		<div><p><?php t("Current position"); ?>:<br>
...
<?php 
	...
	?>
	<p><?php t("First name"); ?><br><input type="text" value="<?php echo $res['u_fir_name']; ?>" name="u_fir_name" size="50"></p><br>
	<p><?php t("Second name"); ?><br><input type="text" value="<?php echo $res['u_sec_name']; ?>" name="u_sec_name" size="50"></p><br>
	<p><?php t("E-mail"); ?><br><input type="text" value="<?php echo $res['users_email']; ?>" name="u_email" size="50"></p><br>
	<?php 
	if($_SESSION['user'] == $res['users_name']){
	?>
	<p><?php t("New password"); ?><br><input type="password" value="<?php echo $res['users_pass']; ?>" name="u_pass" size="40"></p><br>
	<p><?php t("Repeat passwod"); ?><br><input type="password" value="<?php echo $res['users_pass']; ?>" name="u_r_pas" size="40"></p><br>
	<?php
	}
	?>
	<p><?php t("Avatar"); ?><br><?php echo $img; ?><br><input type="file" name="u_img" accept="image/jpeg,image/png,image/gif" ></p><br>
